...
namespace GradientsApp.Maui.Repositories
{
    public class GradientRepository : IGradientRepository
    {
        ...
        public async void UpdateDatabase(LiteDatabase db, Metadata metadata, IDocumentRepository documentRepository)
        {
            var collection = db.GetCollection<Gradient>(nameof(Gradient));
            collection.DeleteAll();

            var categories = await documentRepository.GetDocumentCollection<Category>(metadata.NameSpace, metadata.Categories);
            var gradients = await documentRepository.GetDocumentCollection<Gradient>(metadata.NameSpace, categories.Select(x => x.File).ToArray());
